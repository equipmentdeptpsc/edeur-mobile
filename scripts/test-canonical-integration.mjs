...
const connectivity=read('lib/useConnectivity.ts');
...
check(environment.includes('canonicalEnvironmentDiagnostics')&&environment.includes("demoFallback: 'DISABLED'")&&runtime.includes('resolvedRuntimeMode()')&&runtime.includes("console.log('EDEUR_RUNTIME_CONFIGURATION'"),'runtime diagnostics are safe, reachable once, and Metro-visible');
check(!auth.includes('mockRepository')&&auth.includes('signInWithOperatorPin')&&auth.includes('/api/auth/operator-pin-login'),'canonical auth cannot use mock/PIN identities');
check(auth.includes('AUTH_SERVICE_UNAVAILABLE')&&auth.includes('INVALID_CREDENTIALS')&&auth.includes('ACCOUNT_UNAVAILABLE')&&auth.includes('SESSION_INITIALIZATION'),'canonical auth classifies failures without exposing identities or tokens');
check(loginScreen.includes('getLoginError() ?? (operatorPinFlow ?')&&!loginScreen.includes('Verify credentials and UAT configuration'),'UAT login renders sanitized classified failures instead of the generic diagnostic dead end');
...
check(loginScreen.includes("mode === 'UAT'")&&loginScreen.includes('Login Name')&&loginScreen.includes('Demo PINs: 1234'),'login UI explicitly separates UAT credentials from demo PIN mode');
...
check(connectivity.includes('new Promise<Response>')&&connectivity.includes('CONNECTIVITY_TIMEOUT')&&connectivity.includes('HEARTBEAT_TIMEOUT_MS'),'native connectivity probing settles even when fetch ignores abort');
check(authentication.includes('initialSession()')&&authentication.includes('INITIAL_SESSION')&&authentication.includes('restoreSession')&&authentication.includes('AUTH_INITIAL_SESSION_TIMEOUT_FIRED'),'UAT Supabase session restoration resolves canonical identity and is bounded');
check(authContext.includes("runtime.environment.mode === 'UAT' ? undefined")&&authContext.includes("'INITIALIZING'")&&authContext.includes('restoreOfflineContinuation')&&authContext.includes('applyCanonicalSession')&&authContext.includes('BOOTSTRAP_TIMEOUT'),'auth initialization defers routing until online authorization or bounded offline restoration resolves');
